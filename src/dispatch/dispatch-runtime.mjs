...
export function renderDispatchPrompt({
  ...
  return [
    ...
    '14) Prototype-style asks may generate local artifacts. If Fast.io access is already authenticated, treat it as an available provider, attempt publication, and keep private-or-skip by using only authenticated workspace/share targets. Member-restricted or registered-account Fast.io shares are acceptable; anyone-with-the-link and quickshare-style links are not. Exclude any temporary trailing Ask: helper block from the final saved note and treat it only as optional rewrite guidance.',
...
export function buildEngineLaunchSpec(engine, options = {}) {
  const bootstrapPrompt = [
    ...
    'Prototype-style asks may generate local artifacts. If Fast.io access is already authenticated, treat it as an available provider, attempt publication, and keep private-or-skip by using only authenticated workspace/share targets. Member-restricted or registered-account Fast.io shares are acceptable; anyone-with-the-link and quickshare-style links are not.',
